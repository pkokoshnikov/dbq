...
class DefaultFailMessageStrategyTest {

    @Test
    void failMessageDeletesRecordById() throws Exception {
        var persistenceService = mock(PersistenceService.class);
        when(persistenceService.update(any(String.class), any())).thenReturn(1);
        var strategy = new DefaultFailMessageStrategy(
                new SchemaName("public"),
                new SubscriptionId("test-subscription"),
                persistenceService);

        strategy.failMessage(TestMessageContainers.message(), new RuntimeException("boom"));
