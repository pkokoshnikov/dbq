...
class HistoryFailMessageStrategyTest {

    @Test
    void failMessageEnsuresPartitionThenMovesMessageToHistory() throws Exception {
        ...
        var strategy = new HistoryFailMessageStrategy(
    ...
    @Test
    void failMessageDoesNotUpdateWhenEnsuringPartitionFails() throws Exception {
        var persistenceService = mock(PersistenceService.class);
        var partitionService = mock(QueuePartitionService.class);
        var subscriptionId = new SubscriptionId("test-subscription");
        var strategy = new HistoryFailMessageStrategy(
        ...
        assertThatThrownBy(() -> strategy.failMessage(message, new RuntimeException("boom")))
                .isSameAs(exception);
