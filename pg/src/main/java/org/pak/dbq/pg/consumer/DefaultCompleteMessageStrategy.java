...
public final class DefaultCompleteMessageStrategy implements CompleteMessageStrategy {
    private final SchemaName schemaName;
    private final SubscriptionId subscriptionId;
    private final StringFormatter formatter = new StringFormatter();
    private final Map<String, String> queryCache = new ConcurrentHashMap<>();

    public DefaultCompleteMessageStrategy(SchemaName schemaName, SubscriptionId subscriptionId) {
    ...
    public <T> void completeMessage(ConsumerQueryContext context, MessageContainer<T> messageContainer)
            throws DbqException {
        var query = queryCache.computeIfAbsent("deleteCompletedMessage|" + subscriptionId.id(),
                k -> formatter.execute("""
